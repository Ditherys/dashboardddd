const STORAGE_KEY = "frn-kpi-dashboard-draft-v1";
const KPI_OPTIONS = [
...
function renderSummaryCards(summary, previousSummary) {
  const cards = [
    {
      title: "Overall KPI",
      sub: "Weighted final score",
      value: summary.overallScore.toFixed(2),
      delta: summary.overallScore - previousSummary.overallScore,
      tone: "score"
    },
    {
      title: "Performance",
      sub: "Transfer, Admits, AHT",
      value: summary.performanceScore.toFixed(2),
      delta: summary.performanceScore - previousSummary.performanceScore,
      tone: "score"
    },
    {
      title: "Attendance",
      sub: "KPI score",
      value: summary.attendanceScore.toFixed(2),
      delta: summary.attendanceScore - previousSummary.attendanceScore,
      tone: "score"
    },
    {
      title: "QA Score",
      sub: "KPI score",
      value: summary.qaScore.toFixed(2),
      delta: summary.qaScore - previousSummary.qaScore,
      tone: "score"
    },
    {
      title: "Headcount",
      sub: "Agents in filtered view",
      value: String(summary.count),
      delta: summary.count - previousSummary.count,
      tone: "count"
    }
  ];

  els.summaryCards.innerHTML = cards.map((card) => {
    const delta = formatDelta(card.delta, card.tone === "count" ? 0 : 2);
    const tone = card.delta > 0 ? "up" : card.delta < 0 ? "down" : "flat";
    return `
